...
void Button::update(sf::RenderWindow &window, sf::Event &event) {
    ...
    sf::Vector2f buttonRightHighCorner = {
        this->position.x + this->label.getGlobalBounds().width +
            2 * this->indent.x,
        this->position.y + this->label.getGlobalBounds().height +
            2 * this->indent.y};
    bool mouseInsideButton =
        (static_cast<float>(mousePosition.x) >= buttonLeftLowCorner.x &&
    ...
    // TODO: two types of button: execute on press & execute on release?
    // Possible ButtonState rework?
