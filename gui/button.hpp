...
protected:
    ...
    sf::Text label;
    sf::Vector2f size;  // TODO: resolve label dependency; rn size does nothing
                        // - but it should
...
private:
    sf::RectangleShape shapeUnavailable;  // like if there are no saved games,
                                          // you can't click "Load game"
    sf::RectangleShape shapeAvailable;
    sf::RectangleShape shapeChosen;  // dunno about naming; for situations when
...
public:
    RectangleShapeButton(
        sf::RectangleShape &newShapeUnavailable,
        ...
        : shapeUnavailable(newShapeUnavailable),
          shapeAvailable(newShapeAvailable),
...
private:
    sf::Sprite spriteUnavailable;  // like if there are no saved games, you
                                   // can't click "Load game"
    sf::Sprite spriteAvailable;
    sf::Sprite spriteChosen;  // dunno about naming; for situations when your
                              // cursor is hovering above it
    // or you chose it via arrows/keyboard, but did not activate
    sf::Sprite
        spriteClicked;  // TODO: right now spriteClicked never appears because
                        // of a need to disable ButtonState::Clicked right away
                        // to avoid multiple button executions by 1 click
public:
    [[maybe_unused]] SpriteButton(
        sf::Sprite &newSpriteUnavailable,
